...
| 覆盖维度         | 说明                                                         | 覆盖情况                                       |
|------------------|--------------------------------------------------------------|------------------------------------------------|
| 空/非空          | 各可选参数为 None 与非 None（有效值）                         | 按 API 实际填写：已覆盖 / 未覆盖及原因          |
| 枚举选项         | ReduceOp、backend 模式等离散取值                               | 按 API 实际填写                                |
| 参数类型         | Tensor / int / str / ProcessGroup 等声明支持的类型             | 按 API 实际填写                                |
| 传参与不传参     | 带默认值参数：显式传入与省略走默认                             | 按 API 实际填写                                |
| 等价类/边界值    | 空 tensor、单元素、大 shape、非法 rank 等                    | 按 API 实际填写                                |
| 正常传参场景     | 合法输入下调用成功、返回结构符合预期                           | 按 API 实际填写                                |
| 异常传参场景     | 非法类型、错误 device、不一致 shape 等（需稳定可断言的异常）   | 按 API 实际填写；无稳定异常路径则写未覆盖及原因 |
...
- [ ] 测试文件名不包含 `.`
- [ ] 头部注释包含测试目的、API 名称、**覆盖维度表**（三列：覆盖维度 / 说明 / 覆盖情况，且七行维度与模板对齐：空非空、枚举、参数类型、传参不传参、等价类边界、正常、异常）、未覆盖项说明（简体中文）
